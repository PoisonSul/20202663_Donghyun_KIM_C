...
* BGM은 예를들어 "C:\\Users\\Desktop\\tetris\\음악명" 경로를 문자열로 저장해야합니다.
* 경로는 \ 가 아니라 \\를 해야함.
...
// 음원은 저작권의 문제로 따로 제공되지 않습니다.
// #define BGM "C:\\Users\\Desktop\\tetris\\음악명.wav" //  WAV Full Path
...
/**
* 새로운 블록인가? (flag느낌.)
* 새로운 블록 == true 라면 새로운 블록을 false 처리 한 후에 
...
/**
* 회전 기능의 방향키를 입력 받을 시
* turn(현재 블록의 회전 상태 0,1,2,3w중 예를들어 turn == 0 번째 회전 상태 (초기블록이라면)
...
* 그럴 경우 현재 좌표값이 보드의 바닥과 닿거나, 바닥화 된 블록이라고 판단되기에 
...
/**
* 보드의 특정 가로줄이 블록으로 저장되었는가? IsMaxLine()
* yes? -> deleteLine()_
*/
boolean IsMaxLine();
void deleteLine();
